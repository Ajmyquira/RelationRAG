...
class EmbeddingStore:
    ...
    def insert_strings(self, texts: List[str]):
        ...
        logger.info(f"Inserting {len(missing_ids)} new records, {len(all_hash_ids) - len(missing_ids)} records already exist.")
    ...
    def _load_data(self):
        ...
        self.embedding_array = np.array(self.embeddings, dtype=np.float32)
    ...
    def get_embeddings(self, hash_ids, dtype=np.float32) -> list[np.ndarray]:
        ...
        embeddings = self.embedding_array[indices]
