...
def retrieve_knn(
    ...
    for query_batch, query_batch_start_idx in tqdm(
        ...
        for i in range(final_topk_indices.size(0)):
            ...
            for j in range(len(final_topk_sim_scores_i)):
                if final_topk_sim_scores_i[j] < threshold_score:
                    break

            query_to_topk_key_relative_ids = batch_topk_indices[i][final_topk_indices_i]
            query_to_topk_key_ids = [key_ids[idx] for idx in query_to_topk_key_relative_ids[:j].cpu().numpy()]
            results[query_idx] = (query_to_topk_key_ids, final_topk_sim_scores_i[:j].numpy().tolist())
        # print(len(results))
        # print(len(results[query_idx][0]))
        # print(len(results[query_idx][1]))
        
        # del final_topk_indices
        # del final_topk_sim_scores
        # del batch_topk_sim_scores
        # del batch_topk_indices
